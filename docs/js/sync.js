...
function configured() {
  return Boolean(
    SUPABASE_CONFIG.url && SUPABASE_CONFIG.anonKey && !SUPABASE_CONFIG.url.includes('YOUR_'),
  );
}
...
export class SyncManager {
  ...
  async init() {
    ...
    try {
      ...
      this.onStatus({
        kind: this.user ? 'online' : 'local',
        text: this.user ? '同期可能' : '未ログイン',
      });
      this.client.auth.onAuthStateChange((event, session2) => {
        ...
        if (this.user && (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED'))
          setTimeout(() => this.syncNow().catch(() => {}), 0);
      });
      if (this.user) await this.syncNow();
      window.addEventListener('online', () => this.scheduleSync(200));
      window.addEventListener('offline', () =>
        this.onStatus({ kind: 'offline', text: 'オフライン：端末に保存' }),
      );
    } catch (err) {
      console.error(err);
  ...
  async syncNow() {
    ...
    this.running = this._sync().finally(() => {
      this.running = null;
    });
    return this.running;
  }
  ...
  async _sync() {
    ...
    const remoteAttempts = await fetchAll((from, to) =>
      this.client
        .from('attempts')
        .select('id,question_id,answered_at,correct,choice,domain,mastery_before,mastery_after')
        .eq('user_id', uid)
        .range(from, to),
    );
    const remoteStates = await fetchAll((from, to) =>
      this.client
        ...
        .select('question_id,starred,star_updated_at,updated_at')
        .eq('user_id', uid)
        .range(from, to),
    );
    ...
    const mergedAttempts = [...mergedMap.values()].sort(
      (a, b) => new Date(a.answered_at) - new Date(b.answered_at),
    );

    const remoteIds = new Set(remoteAttempts.map((a) => a.id));
    const uploadAttempts = (local.attempts || [])
      .filter((a) => a?.id && !remoteIds.has(a.id))
      .map((a) => ({ ...a, user_id: uid }));
    if (uploadAttempts.length) {
      const { error } = await this.client
        .from('attempts')
        .upsert(uploadAttempts, { onConflict: 'id' });
      if (error) throw error;
    }

    const starMap = {};
    for (const r of remoteStates) {
      starMap[r.question_id] = {
        starred: Boolean(r.starred),
        star_updated_at: r.star_updated_at || r.updated_at || null,
      };
    }
    for (const [qid, p] of Object.entries(local.progress || {})) {
      ...
      if (!remote) {
        if (p.starred || localTs)
          starMap[qid] = { starred: Boolean(p.starred), star_updated_at: localTs };
        continue;
      }
      if (
        localTs &&
        (!remote.star_updated_at || new Date(localTs) > new Date(remote.star_updated_at))
      ) {
        starMap[qid] = { starred: Boolean(p.starred), star_updated_at: localTs };
      }
    }
    ...
    const snapshots = Object.entries(rebuilt.progress || {}).map(([question_id, p]) => ({
      ...
      star_updated_at: p.star_updated_at || null,
      ...
      recovery_count: p.recovery_count || 0,
      correct_days: p.correct_days || [],
      last_answered_at: p.last_answered_at || null,
      last_wrong_at: p.last_wrong_at || null,
    ...
    if (snapshots.length) {
      const { error } = await this.client
        .from('user_question_state')
        .upsert(snapshots, { onConflict: 'user_id,question_id' });
      if (error) throw error;
    }

    this.onStatus({
      kind: 'online',
      text: `同期済み ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
    });
  }
}
