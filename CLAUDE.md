...
- 現在フェーズ: **381問・285語・5領域・全過去問の選択肢解説が揃った → 実機確認と公開が次**
  ...
  - 2026-08-19: 過去問300問の**選択肢ごとの解説**（1,200件）を `content/past-reasons.json` へ用意し、
    A〜Dすべてに「解説」を表示するようにした。誤答選択肢には「これは何の説明か」を書いてある
- 次アクション:
...
docs/                  ★GitHub Pages の配信ルート。ここだけがブラウザへ配られる
  ...
  js/         app.js  sync.js  supabase-config.js
              ...
              glossary.js（用語辞書ビュー）
              text.js（上付き・下付き・改行の整形）  source.js（★出典と改変の表示）
...
- `npm test` — 履歴移行 / 用語辞書 / 過去問（出典・改変・記法・辞書と解説の整合）の3本。🔥 `docs/js/` を触ったら必ず通す
...
- 🔥 **`docs/` 配下のファイル配置を変えたら `sw.js` の `CACHE` 名を必ず上げる**（現在 `itss-l5-v6`）。
